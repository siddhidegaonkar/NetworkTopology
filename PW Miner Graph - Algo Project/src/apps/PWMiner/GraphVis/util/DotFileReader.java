...
public class DotFileReader extends AbstractGraphReader {
	...
	protected static final HashMap KNOWN_COLORS;
	 ...
	 */
	protected HashMap groupDefaults;
		...
		while(l != null && ! l.trim().equals("}")) {
			...
			if("".equals(l.trim()) || l.matches("\\s*//.*") ||
				...
				if(isNodeLabel(m.group(1))) {
					...
					HashMap attributes = parseAttributes(m.group(2));
					...
					HashMap attributeSet = parseAttributes(attributes);
				...
				if(m.matches()) {
					...
					HashMap attributes = parseAttributes(m.group(3));
	...
	/**
	 * Initializes the default values for known attributes <String, HashMap<String, String>>
	 * @return  
	 */
	private HashMap initializeDefaults() {
		HashMap defaults =new HashMap();
		
		HashMap graphDefaults = new HashMap();
		defaults.put("graph", graphDefaults);
		
		HashMap nodeDefaults = new HashMap();	
		for( int i=0 ; i< NODE_ATTRIBUTES.length; i++) {
			Object[] o = NODE_ATTRIBUTES[i];
			// name in dot, data type, default
		...
		HashMap edgeDefaults = new HashMap();
		for( int j=0 ; j< EDGE_ATTRIBUTES.length; j++){
			Object[] o = EDGE_ATTRIBUTES[j];
			// name in dot, data type, default
	 ...
	 * @param attributes
	 */
	private void setGroupDefaults(String groupname, HashMap attributes) {
		if (Config.print) System.out.println("setting defaults for all of: " + groupname);
		HashMap defaults = (HashMap) groupDefaults.get(groupname);
		if(defaults != null) {
	...
	private String getGroupDefault(String groupName, String attribute) {
		...
		HashMap a = (HashMap)groupDefaults.get(groupName);
		return (String) (a.get((Object)attribute));
	}
	 ...
	 * @param attributes
	 */
	private void setNodeAttributes(Node n, HashMap attributes) {
		for( int i=0 ; i< NODE_ATTRIBUTES.length; i++) {
			Object[] o = NODE_ATTRIBUTES[i];
			// name in dot, data type, default
			...
			if(o[1].equals(int.class)) {
				defaultValue = Integer.parseInt(defv);
			...
			addField(n.getTable(), nameInDot, (Class)o[1], o[2]);//defaultValue
		...
		addField(n.getTable(), "color", int.class, new Color(ColorLib.gray(0)));
		if(color!=null) {
	 ...
	 * @param attributes
	 */
	private void setEdgeAttributes(Edge e, HashMap attributes) {
		for( int j=0 ; j< EDGE_ATTRIBUTES.length; j++){
			Object[] o = EDGE_ATTRIBUTES[j];
			// name in dot, data type, default
			String nameInDot = (String) o[0];
		...
		addField(e.getTable(), "color", int.class, new Color(ColorLib.gray(0)));
		if(color!=null) {
			Color c = getColor(color);
	...
	private Color getColor(String color) {
		Color c = (Color)(KNOWN_COLORS.get(color.toUpperCase()));
		if(c == null) {
	 ...
	 */
	private HashMap parseAttributes(String attributestring) {		
